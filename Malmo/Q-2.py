...
class TabQAgent(object):
    ...
    def updateQTable(self, reward, current_state):
        ...
        td_target = reward + self.discount_factor * max(self.q_table[current_state])
        td_error = td_target - self.q_table[self.prev_s][self.prev_a]

        # assign the new action value to the Q-table
        self.q_table[self.prev_s][self.prev_a] += self.alpha * td_error
    ...
    def make_epsilon_greedy_policy(self):
        def policy_fn(observation):
            ...
            best_action = np.argmax(self.q_table[observation])
    ...
    def act(self, world_state, agent_host, current_r):
        ...
        a = np.random.choice(4, p=self.policy(current_s))
        # rnd = random.random()
        # if rnd < self.epsilon:
    ...
    def drawQ(self, curr_x=None, curr_y=None):
        ...
        if self.canvas is None or self.root is None:
            ...
            self.canvas = tk.Canvas(self.root, width=7 * scale, height=14 * scale, borderwidth=0,
